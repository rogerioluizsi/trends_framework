...
df%>% 
  ggplot(aes(name, aabsa))+ geom_boxplot(outlier.shape = NA) +geom_jitter(aes(colour = year)) +
...
selected = c('NU_CIENCIA_NATUREZA','NU_CIENCIAS_HUMANAS','NU_LINGUAGENS_CODIGOS', 'NU_MATEMATICA', 'DIVERSIDADE', 'IN_FORM_DOCENTE' )
ggplot(df%>%filter(name %in% selected)%>%
         select(mua, year, name), aes(x=as.character(year), y=mua, group=name, color=name))+
  scale_color_viridis_d(labels = labels) + geom_point( size = 1)+ geom_line()#geom_smooth(method=lm, se = FALSE)+ 
  guides(colour = guide_legend(override.aes = list(size = 2)))+theme_publish()+
  ylab("Max Uncentered ALE") +xlab("Years")+ ylim(-0.1, 0.2) + theme(legend.position="right", 
                                                                      ...
                                                                      axis.text.y = element_text(color = 'gray15'))%>%ggplotly
+
  ggtitle(classifier)
...
colnames(corr)<-c("Natural science faculty", "Humanities faculty", 'Languages faculty', "Math faculty",
                 "Faculty work overload", "Faculty adequate training")
rownames(corr)<-c("Natural science faculty", "Humanities faculty", 'Languages faculty', "Math faculty",
                  "Faculty work overload", "Faculty adequate training")
...
histo%>%ggplot(aes(x=grupo, y =mua, color=grupo))+geom_boxplot()+geom_jitter()+
...
track_importance<-histo%>%group_by(year)%>%arrange(desc(aabsa))%>%do(head(., n=10))%>%
  count(year, grupo)%>%group_by(year)%>%mutate(n = n/sum(n))
...
ggplot(aucs, aes(as.character(year), value, shape=metric, linetype=clas, group=interaction(metric, clas)))+
